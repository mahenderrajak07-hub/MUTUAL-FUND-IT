...
async function webSearch(query) {
  ...
  const searchPayload = JSON.stringify({
    ...
    messages: [{
      role: 'user',
      content: `Search for: ${query}\n\nReturn the key financial data found as plain text. Focus on: NAV, 1Y return, 3Y return, 5Y return, Sharpe ratio, Beta, Standard deviation, Alpha, TER/Expense ratio, AUM, Fund manager name and tenure. Be concise.`
    }]
  });
  ...
  const parsed = JSON.parse(result.body);
  if (result.status !== 200) throw new Error(parsed.error?.message || 'Search failed');
...
IMPORTANT: Replace every X using ONLY the live web data provided. Use exact numbers from the live data. Return ONLY the JSON.`;
